...
namespace CycloneDX.Services
{
    ...
    /// </summary>
    public class NugetV2Service : INugetService
    {
        ...
        {
            var packageSource = nugetInput.nugetUsername is null
                ? new PackageSource(nugetInput.nugetFeedUrl)
        ...
        public async Task<Component> GetComponentAsync(string name, string version, Component.ComponentScope? scope)
        {
            ...
            NuspecModel nuspecModel = null;
            try
            {
                nuspecModel = await GetNuspec(name, version, nuspecFilename, resource).ConfigureAwait(false);
            }
            catch
            {
                throw;
            ...
            else
            {
                ...
                if (license == null)
                {
                    ...
                    if (repository != null && !string.IsNullOrWhiteSpace(repository.Url))
                    {
                        ...
                        {
                            license = await _githubService
                                .GetLicenseAsync($"{repository.Url}/blob/{repository.Commit}/licence")
